...
//		).spanningTrees().size should equal(112)
	...
	@Test def `P85 (**) Graph isomorphism.`() {
		...
		fromString("[a-b]").isIsomorphicTo(fromString("[1]")) should equal(false)
		fromString("[a-b, b-c]").isIsomorphicTo(fromString("[a-b, c]")) should equal(false)
		fromString("[a-b, b-c, c-d]").isIsomorphicTo(fromString("[a-b, b-c, c-a, d]")) should equal(false)

		fromString("[a-b]").isIsomorphicTo(fromString("[5-7]")) should equal(true)
		fromString("[a-b, b-c, c-d, d-a]").isIsomorphicTo(fromString("[1-2, 2-3, 3-4, 4-1]")) should equal(true)
	}
...
//		Graph.fromString("[a-b, b-c, a-c, a-d]").colorNodes2().toString() should equal("List((Node(b),3), (Node(d),2), (Node(c),2), (Node(a),1))")
	...
	@Test def `P89 (**) Bipartite graphs.`() {
		Digraph.fromString("[a>b, c>a, d>b]").isBipartite should equal(true)
		Graph.fromString("[a-b, b-c, c-a]").isBipartite should equal(false)
		Graph.fromString("[a-b, b-c, d]").isBipartite should equal(true)
		Graph.fromString("[a-b, b-c, d, e-f, f-g, g-e, h]").isBipartite should equal(false)
	}
