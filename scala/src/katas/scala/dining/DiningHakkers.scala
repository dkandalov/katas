package katas.scala.dining

import katas.scala.dining.DiningHakkers._
...
// Copied from http://klangism.tumblr.com/post/968180337/dining-hakkers
...
class Hakker(name: String, left: ActorRef, right: ActorRef) extends Actor {
	...
	def waiting_for(chopstickToWaitFor: ActorRef, otherChopstick: ActorRef): Receive = {
		case Taken(`chopstickToWaitFor`) =>
			logInfo("%s has picked up %s and %s, and starts to eat", name, left.toString(), right.toString())
			become(eating)
	...
	def eating: Receive = {
		case Think =>
			...
			left ! Put(self)
			right ! Put(self)
			logInfo("%s puts down his chopsticks and starts to think", name)
			scheduleOnce(self, Eat, 5, TimeUnit.SECONDS)
	}

	//All hakkers start in a non-eating state
	override def receive = {
		case Think =>
			logInfo("%s starts to think", name)
			become(thinking)
			scheduleOnce(self, Eat, 5, TimeUnit.SECONDS)
	}
}
...
object DiningHakkers {
	...
	def logInfo(message: String, values: AnyRef*) {
		println(String.format(message, values : _*))
	}
