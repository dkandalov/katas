...
"""
	...
	@Test def shouldDetermineIfQueensAreOnTheSameDiagonal() {
		val row = 7
		val col = 5
		Queen(row, col).notOnTheSameDiagonalAs(Queen(row + 2, col - 2)) should equal(false) // top-right
		Queen(row, col).notOnTheSameDiagonalAs(Queen(row - 2, col - 2)) should equal(false) // top-left
		Queen(row, col).notOnTheSameDiagonalAs(Queen(row - 2, col + 2)) should equal(false) // bottom-left
		Queen(row, col).notOnTheSameDiagonalAs(Queen(row + 2, col + 2)) should equal(false) // bottom-right
	}
