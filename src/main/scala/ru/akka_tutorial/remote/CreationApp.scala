package ru.akka_tutorial.remote
...
class CreationApplication extends Bootable {
  val system = ActorSystem("RemoteCreation", ConfigFactory.load(CalcApp.config).getConfig("remoteCreate").withFallback(CalcApp.config))
  val localActor = system.actorOf(Props[CreationActor], "creationActor")
  val remoteActor = system.actorOf(Props[AdvancedCalculatorActor], "advancedCalc")
