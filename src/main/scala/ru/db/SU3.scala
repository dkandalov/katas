...
class SU3 extends StringUtil {
  ...
  override def reverse(input: String) = input.reverse

  override def getDistribution(input: String) = {
    input.foldLeft(new HashMap[JCharacter, JInteger]()) { (acc, c) =>
      var count = acc.get(c)
  ...
  override def getFirstNSortedChars(input: String, topN: Int) = input.sorted.substring(0, topN)

  override def getUniqueCharsSortedByOccurrence(input: String) = {
    revertedDistributionOf(input).foldLeft("") { _ + _._2 }
  }

  override def getMode(input: String) = {
  ...
  private def revertedDistributionOf(input: String) : List[(JInteger, JCharacter)] = {
    val entries = getDistribution(input).entrySet()
    entries.foldLeft(List[(JInteger, JCharacter)]()) { (acc, e) => (e.getValue, e.getKey) :: acc }.sortBy(-_._1)
  }
}
...
class SU3Test extends ShouldMatchers {
  ...
  @Test def distribution() {
    utils.getDistribution("") should equal(new HashMap())
  ...
  @Test def equalScalaMap() {
    ...
    new HashMap() should equalScalaMap(Map())

    val map = new HashMap[JCharacter, JInteger]()
    map.put('a', 1)
...
//    map should equalScalaMap(Map('b' -> 1)) // should fail
  ...
  def equalScalaMap(right: Map[Char, Int]): Matcher[Any] = {
    val rightMap = right.foldLeft(new HashMap[JCharacter, JInteger]) { (acc, e) =>
      acc.put(e._1, e._2)
      acc
    }
    equal(rightMap)
